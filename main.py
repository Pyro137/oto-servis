...
class AracKartiFormu(QDialog):
    ...
    def initUI(self):
        ...
        self.telefon_edit.setReadOnly(True)
        cari_tipi_label = QLabel('Cari Tipi')
        self.cari_tipi_edit = QLineEdit()
        self.cari_tipi_edit.setReadOnly(True)
        ...
        cari_layout.addWidget(telefon_label, 2, 0)
        cari_layout.addWidget(self.telefon_edit, 2, 1, 1, 2)
        cari_layout.addWidget(cari_tipi_label, 3, 0)
        cari_layout.addWidget(self.cari_tipi_edit, 3, 1, 1, 2)
    ...
    def cari_sec(self):
        ...
        def on_cari_select(kod, ad, telefon, cari_tipi):
            ...
            self.telefon_edit.setText(telefon)
            self.cari_tipi_edit.setText(cari_tipi)
...
class CariSecDialog(QDialog):
    ...
    def aktar(self):
        ...
        telefon = self.table.item(row, 2).text()
        cari_tipi = self.table.item(row, 3).text()
        
        # Eğer callback fonksiyonu varsa çağır
        if self.on_select:
            self.on_select(self.selected_cari_kodu, self.selected_cari_adi, telefon, cari_tipi)
...
class CariAracListesiDialog(QDialog):
    ...
    def load_arac_data(self):
        """Araç verilerini veritabanından yükler"""
        araclar = self.db.get_all_araclar()
        self.table.setRowCount(len(araclar))
        
        for row, arac in enumerate(araclar):
            for col, value in enumerate(arac):
                item = QTableWidgetItem(str(value))
                self.table.setItem(row, col, item)
        self.alt_label.setText(f'{len(araclar)} adet kayıt listeleniyor')
...
class ServisGirisiEkleDialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.selected_cari_kodu = None
        self.selected_plaka = None
        self.db = Database()
        self.islemler = []  # İşlemleri tutacak liste
        self.initUI()
    ...
    def initUI(self):
        ...
        self.telefon_edit = QLineEdit()
        self.cari_tipi_combo = QComboBox()
        self.cari_tipi_combo.addItems(['Müşteri', 'Tedarikçi'])
        form_grid.addWidget(QLabel('Cari Kodu'), 0, 0)
        form_grid.addWidget(self.cari_kodu_edit, 0, 1)
        form_grid.addWidget(QLabel('Cari Adı / Ünvanı'), 1, 0)
        form_grid.addWidget(self.cari_adi_edit, 1, 1)
        form_grid.addWidget(QLabel('Telefon'), 2, 0)
        form_grid.addWidget(self.telefon_edit, 2, 1)
        form_grid.addWidget(QLabel('Cari Tipi *'), 3, 0)
        form_grid.addWidget(self.cari_tipi_combo, 3, 1)
    ...
    def cari_sec(self):
        def on_cari_select(kod, ad, telefon, cari_tipi):
            ...
            self.telefon_edit.setText(telefon)
            self.cari_tipi_combo.setCurrentText(cari_tipi)
            self.selected_cari_kodu = kod
        
        dialog = CariSecDialog(self, on_select=on_cari_select)
        dialog.exec_()
    def arac_sec(self):
        if not self.selected_cari_kodu:
            QMessageBox.warning(self, 'Uyarı', 'Lütfen önce bir cari seçin!')
            return

        def aktar_callback(plaka, arac_tipi, model_yili, marka, model):
            # Plaka
            self.plaka_edit.setText(plaka)

            # Araç tipi – önce listede var mı bak
            idx = self.arac_tipi_combo.findText(arac_tipi, Qt.MatchFixedString)
            if idx != -1:
                self.arac_tipi_combo.setCurrentIndex(idx)
            else:
                self.arac_tipi_combo.addItem(arac_tipi)
                self.arac_tipi_combo.setCurrentIndex(self.arac_tipi_combo.count() - 1)

            # Yıl / Marka / Model
            self.model_yili_edit.setText(str(model_yili))
            self.marka_edit.setText(marka)
            self.model_edit.setText(model)

            # Seçilen plaka ve geçmiş servisler
            self.selected_plaka = plaka
            self.gecmis_kayitlari_yukle()

        dialog = AracSecDialog(self, on_select=aktar_callback,
                            cari_kodu=self.selected_cari_kodu)
        dialog.exec_()
    def gecmis_kayitlari_yukle(self):
        """Seçili araca ait geçmiş servis kayıtlarını yükler"""
        if not self.selected_plaka:
            return
            
        # Veritabanından geçmiş kayıtları al
        kayitlar = self.db.get_servis_kayitlari_by_arac_and_status(self.selected_plaka, 'Kapalı')
...
class OdemeHareketleriDialog(QDialog):
    ...
    def load_data(self):
        """Ödeme hareketlerini veritabanından yükler"""
        start_date = self.tarih1.date().toString('yyyy-MM-dd')
        end_date = self.tarih2.date().toString('yyyy-MM-dd')
        
        hareketler = self.db.get_odeme_hareketleri_by_date_range(start_date, end_date)
        self.table.setRowCount(len(hareketler))
        
        toplam_tutar = 0
        for row, hareket in enumerate(hareketler):
            for col, value in enumerate(hareket):
                item = QTableWidgetItem(str(value))
        ...
        self.alt_label.setText(
            f'{len(hareketler)} adet kayıt listeleniyor | '
            f'Toplam Tutar: {toplam_tutar:,.2f} TL | '
            f'{self.tarih1.date().toString("d.MM.yyyy")} - {self.tarih2.date().toString("d.MM.yyyy")} '
            f'Tarihleri arasında {self.tarih1.date().daysTo(self.tarih2.date())} günlük kayıt sonuçları.'
        )
...
class AracGecmisiDialog(QDialog):
    ...
    def load_data(self):
        """Araç geçmişini veritabanından yükler"""
        gecmis = self.db.get_arac_gecmisi()
        self.table.setRowCount(len(gecmis))
        
        for row, kayit in enumerate(gecmis):
            for col, value in enumerate(kayit):
                item = QTableWidgetItem(str(value))
                self.table.setItem(row, col, item)
        
        self.alt_label.setText(f'{len(gecmis)} adet araç geçmişi listeleniyor')
...
class AracSecDialog(QDialog):
    def __init__(self, parent=None, on_select=None, cari_kodu=None):
        super().__init__(parent)
        self.setWindowTitle('Araç Listesi')
    ...
    def initUI(self):
        ...
        self.search_edit.textChanged.connect(self.filter_arac)
        ana_layout.addWidget(self.search_edit)
        ...
        btn_iptal.clicked.connect(self.reject)
        btn_aktar.clicked.connect(self.aktar)
        btn_yeni.clicked.connect(self.yeni_arac_ekle)

        self.setLayout(ana_layout)
        self.load_arac_data()

    def load_arac_data(self):
        """Seçili cariye ait araçları yükler"""
        if not self.cari_kodu:
            return
            
        araclar = self.db.get_arac_by_cari(self.cari_kodu)
        self.table.setRowCount(len(araclar))
        
        for row, arac in enumerate(araclar):
            # plaka, arac_tipi, model_yili, marka, model
            self.table.setItem(row, 0, QTableWidgetItem(str(arac[0])))  # plaka
            self.table.setItem(row, 1, QTableWidgetItem(str(arac[1])))  # arac_tipi
            self.table.setItem(row, 2, QTableWidgetItem(str(arac[2])))  # model_yili
            self.table.setItem(row, 3, QTableWidgetItem(str(arac[3])))  # marka
            self.table.setItem(row, 4, QTableWidgetItem(str(arac[4])))  # model

    def filter_arac(self):
        """Araç listesini filtreler"""
        search_text = self.search_edit.text().lower()
        for row in range(self.table.rowCount()):
            show_row = False
            for col in range(self.table.columnCount()):
    ...
    def aktar(self):
        ...
        arac_tipi  = self.table.item(row, 1).text()
        model_yili = self.table.item(row, 2).text()
        marka      = self.table.item(row, 3).text()
        model      = self.table.item(row, 4).text()
        
        if self.on_select:
            self.on_select(plaka, arac_tipi, model_yili, marka, model)
        
        self.accept()

    def yeni_arac_ekle(self):
        """Yeni araç ekleme dialogunu açar"""
        dialog = AracKartiFormu(self)
        if dialog.exec_() == QDialog.Accepted:
            self.load_arac_data()  # Refresh the list after adding new vehicle
