...
class CariServisKayitlariDialog(QDialog):
    ...
    def show_detay(self):
        ...
        kayit_detay = self.cari_bilgi.copy()
...
class ServisGirisiEkleDialog(QDialog):
    ...
    def cari_sec(self):
        def aktar_callback(kod, ad):
            self.cari_kodu_edit.setText(kod)
            self.cari_adi_edit.setText(ad)
        dialog = CariSecDialog(self, on_select=aktar_callback)
        dialog.exec_()
...
class OdemeAlmaFormuDialog(QDialog):
    ...
    def initUI(self):
        ...
        odeme_giris_layout.addWidget(QLabel('Cari Bilgilerini Giriniz'), 0, 0)
        ...
        btn_ekle = QPushButton('Ekle')
        odeme_giris_layout.addWidget(btn_ekle, 4, 2)
    ...
    def kaydet(self):
        # Burada ödeme kayıt işlemleri yapılabilir
...
class CariSecDialog(QDialog):
    ...
    def initUI(self):
        ...
        self.table.setHorizontalHeaderLabels(['Cari Kodu', 'Cari Adı / Ünvanı'])
        self.table.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)
        self.table.setEditTriggers(QTableWidget.NoEditTriggers)
        self.table.setSelectionBehavior(QTableWidget.SelectRows)
        self.table.setSelectionMode(QTableWidget.SingleSelection)
